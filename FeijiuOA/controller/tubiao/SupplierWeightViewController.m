...
#import "SupplierWeightViewController.h"
#import "BaoBiaoWebApi.h"
#import "SupplierWeightCell.h"
#import "SupplierWeightView.h"
#import "SupplierWeightModel.h"
#import "AlertHelper.h"
#import "SelectItemViewController.h"
#import "AppDelegate.h"
#import "URL.h"
#define tittleWidth 480
@interface SupplierWeightViewController ()
<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>
{
    NSMutableArray *dataArr;
    UIScrollView *rightScrollView;
    UITableView *rightTableView;
    SupplierWeightView *view2;
    NSString *hwmcc;
    NSString *gyss;
     int indexy;
}
...
@implementation SupplierWeightViewController
...
- (void)viewDidLoad {
    ...
    view2 = [[SupplierWeightView alloc]instanceChooseView];
...
-(void)getDataWithTime:(NSString *)time
{
    ...
    _lab_tittle.text=@"供应商总个数:0 货物个数:0 总重量:0";
    [AlertHelper MBHUDShow:@"加载中..." ForView:self.view AndDelayHid:30];
    [BaoBiaoWebApi supplierweightstatisticsWithDate:time hwmc:hwmcc gys:gyss method:method Success:^(NSArray *arr) {
        [AlertHelper hideAllHUDsForView:self.view];
      //  NSLog(@"SSSS %@",arr);
        @try {
            
            for (NSDictionary *dict in arr[0][@"lls"]) {
                SupplierWeightModel *model=[[SupplierWeightModel alloc]init];
            ...
            if (data.count>0) {
                ...
                [str appendFormat:@"供应商总个数:%@ 货物个数:%@ 总重量:%@ ",dic[@"zgys"],dic[@"hwgs"],dic[@"zzl"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SupplierWeightCell *cell = [tableView dequeueReusableCellWithIdentifier:identfier];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"SupplierWeightCell" owner:self options:nil] lastObject];
        
    }
    SupplierWeightModel *model=[dataArr objectAtIndex:indexPath.row];
