...
#import "CK_SupplierAmountViewController.h"
#import "Baobiao_ckWebApi.h"
#import "CK_SupplierAmountCell.h"
#import "CK_SupplierAmountView.h"
#import "CK_SupplierAmountModel.h"
...
@interface CK_SupplierAmountViewController ()
<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>
{
    NSMutableArray *dataArr;
    UIScrollView *rightScrollView;
    UITableView *rightTableView;
    CK_SupplierAmountView *view2;
    NSString *hwmcc;
    NSString *zdmcc;
    NSString *khjcc;
    int indexy;
...
@implementation CK_SupplierAmountViewController
...
- (void)viewDidLoad {
    ...
    view2 = [[CK_SupplierAmountView alloc]instanceChooseView];
...
-(void)getDataWithTime:(NSString *)time
{
   ...
   _lab_tittle.text=@"客户总个数:0 货物个数:0 总金额:0 ";
    [AlertHelper MBHUDShow:@"加载中..." ForView:self.view AndDelayHid:30];
    [Baobiao_ckWebApi ck_supplieramountstatisticsWithDate:time  kh:khjcc hwmc:hwmcc method:method Success:^(NSArray *arr) {
        [AlertHelper hideAllHUDsForView:self.view];
      //  NSLog(@"SSSS %@",arr);
        @try {
            
            for (NSDictionary *dict in arr[0][@"lls"]) {
                CK_SupplierAmountModel *model=[[CK_SupplierAmountModel alloc]init];
            ...
            if (data.count>0) {
               NSDictionary *dic=arr[0][@"tj"][0];
                ...
                [str appendFormat:@"客户总个数:%@ 货物个数:%@ 总金额:%@ ",dic[@"zkh"],dic[@"hwgs"],dic[@"zje"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CK_SupplierAmountCell *cell = [tableView dequeueReusableCellWithIdentifier:identfier];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"CK_SupplierAmountCell" owner:self options:nil] lastObject];
        
    }
    CK_SupplierAmountModel *model=[dataArr objectAtIndex:indexPath.row];
